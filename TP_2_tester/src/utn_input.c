...
static int isLetter(char* pResultado, int len)
{
	...
	while(pResultado[i]!='\0')
	{
		...
		if((pResultado[i] < 'A' || pResultado[i] > 'Z') && (pResultado[i] < 'a' || pResultado[i] > 'z') && (pResultado[i]!=' ') && (pResultado[i]!='(' && pResultado[i]!=')') )
			{
				ret = 0;
			}
		i++;
...
int utn_getShort(short int* pResultado, char* mensaje, char* mensajeError, int minimo, int maximo, int reintentos)
{
	short aux;
	int ret=-1;
		...
		if(pResultado!=NULL && mensaje!=NULL && mensajeError!=NULL && minimo<=maximo && reintentos>=0)
		{
			while(reintentos>0)
			{
				reintentos--;
				printf("%s",mensaje);
				if(getShortInt(&aux) == 0 && aux >= minimo && aux <= maximo)
				{
					*pResultado = aux;
					ret=0; // salio bien
					break;
	...
	int ret=-1;
		...
		if(pResultado!=NULL && len>0 && mensaje!=NULL && mensajeError!=NULL && reintentos>=0)
		{
			while(reintentos>0)
			{
				...
				fgets(aux,sizeof(aux),stdin);
				aux[strlen(aux)-1]='\0';
				if(aux[0]!='\0' && isLetter(aux, len)==-1 && strlen(aux)<len) // que solo sean letras y no numeros
				{
					ret=0;
					strcpy(pResultado, aux);
					break;
	...
	int ret=-1;
		...
		if(pResultado!=NULL && len>0 && mensaje!=NULL && mensajeError!=NULL && reintentos>=0)
		{
			while(reintentos>0)
			{
				...
				aux[strlen(aux)-1]='\0';
				utn_toUpper(aux, len);
				if((isLetter(aux, len)==-1 && strlen(aux)<len) && (strncmp(aux,"ARQUERO",len)==0 || strncmp(aux,"DEFENSOR",len)==0 || strncmp(aux,"MEDIOCAMPISTA",len)==0 || strncmp(aux,"DELANTERO",len)==0))
				{
					ret=0;
					strcpy(pResultado, aux);
					break;
